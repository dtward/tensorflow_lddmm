...
Okay getting started version 2.

Recall that placeholders can have a size of None on some dimensions.

Let's work out the sizes I need.

The output of the model should be points

nTimes x nPoints x nDims

This is also the size of the input targets.

The parameters to optimze are the sine and cosine parameter vectors, as well as p0

p0 is size nPoints x nDims

s and c are size nHarmonics x nPoints x nDims

Input times are size nTimes

p1 is size nTimes x nPoints x nDims

To go from s and c to p1 we need a matrix of size nTimes x nHarmonics

So currently I have my epdif equations defined for one set of points and one set of momenta

I need to define it to work for a batch

I don't see how I can do better than just looping, maybe do tf.concat to put back together

https://www.tensorflow.org/api_docs/python/tf/concat

WORKING HERE, THIS ONE IS NOT WORKING YET

i need to re implement epdiff to work with batch!
...
# this will be a variable shape, because I can feed in one time or several times
# this is important, this size will be variable
...
# I need to do the above all in one tensor
#one_to_M = 
raise Exception   
