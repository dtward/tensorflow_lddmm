''' 
in timeseries 2 we estimate a center point
'''
...
niter = 2000
epsilonp = 1.0e-5
epsilont = 1.0e-4 # I had to shrink this when moving to 5 points
...
ppdott = []
for i in range(nT):
    qt_,pt_,qdott_,pdott_ = epd.EPDiffPointsGausianKernel(qt[-1],p1*(float(tvalue[i])-t0),sigmaV,ntintegrate,T)
    qqt.append(qt_)
    ppt.append(pt_)
...
# optimize
train_p = tf.train.GradientDescentOptimizer(epsilonp).minimize(error,var_list=[p0,p1])
train_t = tf.train.GradientDescentOptimizer(epsilont).minimize(error,var_list=[t0])
...
with tf.Session() as sess:
    ...
    for _ in range(niter):
        to_evaluate = [train_p,train_t,error]
    ...
    print('t0 is {}'.format(sess.run(t0,feed_dict=feed_dict)))
